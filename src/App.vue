                ...
                <div v-if="order.historicalTasks && order.historicalTasks.length > 0" class="history-block">
                  ...
                  <div v-for="(task, index) in order.historicalTasks" :key="task.taskId" class="history-item">
                    ...
                    <div class="history-actions">
                      <a :href="getTaskUrl(task.taskId, 'image')" target="_blank" class="btn-action-ghost">看图</a>
                      <button @click="downloadTaskFile(task.taskId, 'image')">存图</button>
                      <a :href="getTaskUrl(task.taskId, 'video')" target="_blank" class="btn-action-ghost">看视频</a>
                      <button @click="downloadTaskFile(task.taskId, 'video')">存视频</button>
                    </div>
                  </div>
                </div>
                <div v-else-if="order.taskId" class="history-block">
                  <div class="history-actions">
                    <a :href="getTaskUrl(order.taskId, 'image')" target="_blank" class="btn-action-ghost">预览照片</a>
                    <button @click="downloadTaskFile(order.taskId, 'image')">下载照片</button>
                    <a :href="getTaskUrl(order.taskId, 'video')" target="_blank" class="btn-action-ghost">预览视频</a>
...
const downloadTaskFile = async (taskId, type) => {
  try {
    ...
    const { data } = await http.get(`/video/tasks/${taskId}/${type}?download=true`, {
      responseType: 'blob'
    });
...
const onCoverImageChange = (e) => {
  ...
  makerForm.coverImageFile = file || null;
  if (localImagePreview.value) {
    URL.revokeObjectURL(localImagePreview.value);
  }
  if (file) {
...
const generateVideo = async () => {
  ...
  try {
    ...
    if (makerTemplate.value.templateType === 'video') {
      ...
      formData.append("templateId", makerTemplate.value.id);
      if (currentOrderId.value) formData.append("orderId", currentOrderId.value);
      formData.append("name", makerForm.name);
      formData.append("age", makerForm.age);
      formData.append("time", makerForm.time);
      if (makerForm.hotel) formData.append("hotel", makerForm.hotel);
      if (makerForm.coverImageFile) formData.append("coverImage", makerForm.coverImageFile);
...
.history-actions button, .history-actions a { display: inline-block; box-sizing: border-box; background: #fff; border: 1px solid #ddd; padding: 2px 8px; border-radius: 4px; font-size: 11px; color: #555; cursor:pointer; text-decoration: none; line-height: 1.5; }
